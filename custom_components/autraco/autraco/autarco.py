...
class Autarco:
    ...
    async def solar(self, public_key: str) -> Solar:
        ...
        data = await self._request(f"{public_key}/energy")
        dataPower = await self._request(f"{public_key}/power")

        return Solar.from_json(data,dataPower)
    ...
    async def account(self, public_key: str) -> Account:
        ...
        data = await self._request(f"{public_key}/site")
